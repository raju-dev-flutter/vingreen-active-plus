...
class ProformaInvoiceEditBloc {
  ...
  Future<void> fetchProformaInvoiceDetailLoaded(
      dynamic proformaInvoiceId) async {
    ...
    dynamic response2 = await proformaInvoiceRepositories
        .fetchProformaInvoiceEditDetailApi(userId, proformaInvoiceId);
    ...
    final proformaInviceDetail = proformaInvoiceEdit.getProformaInvoice;
    // Clients List
    _clientsList.sink.add(proformaInvoice.clientsList!);
    debugPrint('Clients List : ${proformaInviceDetail!.clientName}');
    for (int i = 0; i < _clientsList.value.length; i++) {
      if (_clientsList.value[i].clientName == proformaInviceDetail.clientName) {
    ...
    debugPrint(proformaInviceDetail.dateIssue!);
    List<String> splitIssueDateTime =
        proformaInviceDetail.dateIssue.toString().split('-');
    _seletedDateIssue.sink.add(proformaInviceDetail.dateIssue!);
    _dateIssue.sink.add(DateTime(int.parse(splitIssueDateTime[0]),
        int.parse(splitIssueDateTime[1]), int.parse(splitIssueDateTime[2])));

    debugPrint(proformaInviceDetail.dateDue!);
    List<String> splitDueDateTime =
        proformaInviceDetail.dateDue.toString().split('-');
    _seletedDueDate.sink.add(proformaInviceDetail.dateDue!);
    _dueDate.sink.add(DateTime(int.parse(splitDueDateTime[0]),
        int.parse(splitDueDateTime[1]), int.parse(splitDueDateTime[2])));

    _gstCheckboxValue.sink.add(proformaInviceDetail.igstType!);

    _productsList.sink.add(proformaInvoice.productsList!);
    _filterProduct.sink.add(proformaInvoice.productsList!);
  }
  ...
  Future<void> uploadProformaInvoiceProductUpdate(
      BuildContext _,
    ...
    proformaInvoiceCrudCubit.uploadProformaInvoiceEditSubmit(
