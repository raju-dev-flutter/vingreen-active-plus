...
class QuotationAddBloc {
  ...
  Future<void> fetchQuotationDetailLoaded() async {
    final userId = await userRepositories.hasUserId();
    dynamic response = await quotationRepositories.fetchQuotationAddApi(userId);
    final quotation = QuotationModel.fromJson(response);
    debugPrint(quotation.clientsList?.first.clientName);

    _clientsList.sink.add(quotation.clientsList!);
    _clientsListInit.sink.add(quotation.clientsList!.first);
  ...
  Future<void> uploadQuotationProductUpdate(BuildContext _,
    ...
    quotationCrudCubit.uploadQuotationAddSubmit(
