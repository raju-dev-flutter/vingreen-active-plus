...
import '../../../data/models/proforma_invoice_edit_model.dart';
...
import '../../../logic/rxdart/proforma_invoice/proforma_invoice_edit_bloc.dart';
import '../../components/app_loader.dart';
import '../../components/app_toast.dart';

class ProformaInvoiceEditPage extends StatefulWidget {
  static const String id = "proforma_invoice_edit_page";

  final int proformaInvoiceId;

  const ProformaInvoiceEditPage({Key? key, required this.proformaInvoiceId})
      : super(key: key);

  @override
  State<ProformaInvoiceEditPage> createState() =>
      _ProformaInvoiceEditPageState();
}

class _ProformaInvoiceEditPageState extends State<ProformaInvoiceEditPage> {
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();

  final proformaInvoiceEditBloc = ProformaInvoiceEditBloc();
  late ProformaInvoiceCrudCubit proformaInvoiceCrudCubit;
  ...
  void initState() {
    ...
    proformaInvoiceCrudCubit =
        BlocProvider.of<ProformaInvoiceCrudCubit>(context);
    proformaInvoiceCrudCubit
        .initialProformaInvoiceEditCrud(widget.proformaInvoiceId);
    proformaInvoiceEditBloc
        .fetchProformaInvoiceDetailLoaded(widget.proformaInvoiceId);
    initialMultiTextController();
  }

  Future<void> initialMultiTextController() async {
    debugPrint("=============${widget.proformaInvoiceId}");
    final userId = await userRepositories.hasUserId();
    dynamic response1 =
        await proformaInvoiceRepositories.fetchProformaInvoiceAddApi(userId);
    dynamic response2 = await proformaInvoiceRepositories
        .fetchProformaInvoiceEditDetailApi(userId, widget.proformaInvoiceId);

    debugPrint("================\n${response2.toString()}\n================");

    final proformaInvoice = ProformaInvoiceModel.fromJson(response1);
    final proformaInvoiceEdit = ProformaInvoiceEditModel.fromJson(response2);

    List<GetAttachments> attachments = proformaInvoiceEdit.getAttachments!;
    for (int i = 0; i < attachments.length; i++) {
      var productName = "";
      for (int j = 0; j < proformaInvoice.productsList!.length; j++) {
        if (attachments[i].product.toString() ==
            proformaInvoice.productsList![j].productId.toString()) {
          productName = proformaInvoice.productsList![j].productName!;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<ProformaInvoiceCrudCubit, ProformaInvoiceCrudState>(
        listener: (context, state) {
          if (state is ProformaInvoiceCrudFromSuccess) {
            showSuccessToast(context, "Quotation Successfully Added");
            Navigator.pop(context);
          }
          if (state is ProformaInvoiceCrudFromFailed) {
            ...
            child: Column(
              children: [
                streamBuilderDropdown(
                  label: 'Client Name',
                  streamList: proformaInvoiceEditBloc.clientsList,
                ),
      ...
      bottomNavigationBar:
          BlocBuilder<ProformaInvoiceCrudCubit, ProformaInvoiceCrudState>(
        builder: (context, state) {
          return BottomAppBar(
            child: Container(
              height: 90,
              ...
              child: state is ProformaInvoiceCrudFromLoading
                  ? showDropdownLoaded()
                  : submitButton(onTap: () {
                      proformaInvoiceEditBloc
                          .uploadProformaInvoiceProductUpdate(context,
                              widget.proformaInvoiceId, multiTextController);
                    }),
            ),
          );
        },
  ...
  appBar(BuildContext context) {
    return AppBar(
      ...
      title: Text('Proforma Invoice Edit',
  ...
  getStreamInitValue(String label) {
    if (label == "Client Name") {
      return proformaInvoiceEditBloc.clientsListInit.valueOrNull?.clientName;
    }
  }
              ...
              if (snapshot.hasData) {
                ...
                return DropDownTextField(
                  ...
                  onChanged: (dynamic val) {
                    if (val != '') {
                      if (label == "Client Name") {
                        proformaInvoiceEditBloc.invoiceClientsList(val.value);
                      }
                    }
        ...
        children: [
          ...
          StreamBuilder<DateTime>(
            stream: label == "Issue Date"
                ? proformaInvoiceEditBloc.dateIssue
                : proformaInvoiceEditBloc.dueDate,
            builder: (context, snapshot) {
              final selectedDate = snapshot.data ?? DateTime.now();
              final selectDate = label == "Issue Date"
                  ? proformaInvoiceEditBloc.seletedDateIssue
                  : proformaInvoiceEditBloc.seletedDueDate;
              return InkWell(
                onTap: () async {
                  ...
                  if (pickedDate != null) {
                    if (label == "Issue Date") {
                      proformaInvoiceEditBloc.selectIssueDate(pickedDate);
                    } else {
                      proformaInvoiceEditBloc.selectDueDate(pickedDate);
                    }
                  }
                },
                child: Container(
  ...
  checkBox() {
    final isSelectGst = proformaInvoiceEditBloc.gstCheckboxValue.valueOrNull;
    return Container(
      height: 50,
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      child: Row(
        ...
        children: [
          ...
          InkWell(
            onTap: () {
              setState(() => proformaInvoiceEditBloc.setGstCheckBoxValue(1));
            },
            child: Row(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.center,
          ...
          InkWell(
            onTap: () {
              setState(() => proformaInvoiceEditBloc.setGstCheckBoxValue(2));
            },
            child: Row(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.center,
        ...
        SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                onChanged: (value) =>
                    proformaInvoiceEditBloc.filterProductList(value),
              ...
              Expanded(
                child: StreamBuilder<List<ProductsList>>(
                    stream: proformaInvoiceEditBloc.filterProduct,
                    builder: (_, snapshot) {
                          ...
                          return Padding(
                            ...
                            child: InkWell(
                              onTap: () {
                                ...
                                proformaInvoiceEditBloc.filterProductList('');
                                Navigator.pop(context);
                              },
                              child: Container(
