...
import '../../../logic/rxdart/quotation/quotation_edit_bloc.dart';
import '../../components/app_loader.dart';
import '../../components/app_toast.dart';

class QuotationEditPage extends StatefulWidget {
  static const String id = "quotation_edit_page";

  final int quotationId;

  const QuotationEditPage({Key? key, required this.quotationId})
      : super(key: key);
...
class _QuotationEditPageState extends State<QuotationEditPage> {
  ...
  late QuotationCrudCubit quotationCrudCubit;
  ...
  void initState() {
    super.initState();
    quotationCrudCubit = BlocProvider.of<QuotationCrudCubit>(context);
    quotationCrudCubit.initialQuotationCrud();
    quotationEditBloc.fetchQuotationEditDetailsLoaded(widget.quotationId);
  ...
  Future<void> initialMultiTextController() async {
    ...
    dynamic response2 = await quotationRepositories.fetchQuotationEditDetailApi(
        userId, widget.quotationId);
    final quotation = QuotationModel.fromJson(response1);
    final quotationEdit = QuotationEditModel.fromJson(response2);

    List<GetAttachments> attachments = quotationEdit.getAttachments!;
    for (int i = 0; i < attachments.length; i++) {
      var productName = "";
      for (int j = 0; j < quotation.productsList!.length; j++) {
        if (attachments[i].product.toString() ==
            quotation.productsList![j].productId.toString()) {
          productName = quotation.productsList![j].productName!;
        }
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<QuotationCrudCubit, QuotationCrudState>(
          listener: (context, state) {
        if (state is QuotationCrudFromSuccess) {
          showSuccessToast(context, "Quotation Successfully Added");
          Navigator.pop(context);
        }
        if (state is QuotationCrudFromFailed) {
          showWarningToast(context, state.error);
          Navigator.pop(context);
        }
      }, builder: (context, state) {
        if (state is QuotationCrudLoading) {
          ...
          child: Column(
            children: [
              streamBuilderDropdown(
                label: 'Client Name',
                streamList: quotationEditBloc.clientsList,
              ),
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    streamDataPicker(
                        label: 'Issue Date',
                        streamDate: quotationEditBloc.dateIssue),
      ...
      bottomNavigationBar: BlocBuilder<QuotationCrudCubit, QuotationCrudState>(
        builder: (context, state) {
          return BottomAppBar(
            child: Container(
              height: 90,
              ...
              child: state is QuotationCrudFromLoading
                  ? showDropdownLoaded()
                  : submitButton(onTap: () {
                      quotationEditBloc.uploadQuotationProductUpdate(
                          context, widget.quotationId, multiTextController);
                    }),
            ),
          );
        },
  ...
  getStreamInitValue(String label) {
    if (label == "Client Name") {
      return quotationEditBloc.clientsListInit.valueOrNull?.clientName;
    }
  }
              ...
              if (snapshot.hasData) {
                ...
                return DropDownTextField(
                  ...
                  onChanged: (dynamic val) {
                    if (val != '') {
                      if (label == "Client Name") {
                        quotationEditBloc.quotationClientsList(val.value);
                      }
                    }
  ...
  streamDataPicker(
      {required String label, required ValueStream<DateTime> streamDate}) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          StreamBuilder<DateTime>(
            stream: label == "Issue Date"
                ? quotationEditBloc.dateIssue
                : quotationEditBloc.dueDate,
            builder: (context, snapshot) {
              final selectedDate = snapshot.data;
              final selectDate = label == "Issue Date"
                  ? quotationEditBloc.seletedDateIssue
                  : quotationEditBloc.seletedDueDate;
              return InkWell(
                onTap: () async {
                  ...
                  if (pickedDate != null) {
                    if (label == "Issue Date") {
                      quotationEditBloc.selectIssueDate(pickedDate);
                    } else {
                      quotationEditBloc.selectDueDate(pickedDate);
                    }
                  }
                },
                child: Container(
  ...
  checkBox() {
    final isSelectGst = quotationEditBloc.gstCheckboxValue.valueOrNull;
    return Container(
      height: 50,
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      child: Row(
        ...
        children: [
          ...
          InkWell(
            onTap: () {
              setState(() {
                quotationEditBloc.setGstCheckBoxValue(1);
                debugPrint(isSelectGst.toString());
              });
            },
            child: Row(
              mainAxisAlignment: MainAxisAlignment.start,
          ...
          InkWell(
            onTap: () {
              setState(() {
                quotationEditBloc.setGstCheckBoxValue(2);
                debugPrint(isSelectGst.toString());
              });
            },
            child: Row(
            ...
            child: InkWell(
              ...
              onTap: () {
                for (var i = 0; i < multiTextController.length; i++) {
                  if (position == i) {
                    setState(() {
                      multiTextController.removeAt(i);
                    });
                    break;
                  }
                }
        ...
        SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                onChanged: (value) =>
                    quotationEditBloc.filterProductList(value),
              ...
              Expanded(
                child: StreamBuilder<List<ProductsList>>(
                    stream: quotationEditBloc.filterProduct,
                    builder: (_, snapshot) {
                          ...
                          return Padding(
                            ...
                            child: InkWell(
                              onTap: () {
                                ...
                                quotationEditBloc.filterProductList('');
                                Navigator.pop(context);
                              },
                              child: Container(
